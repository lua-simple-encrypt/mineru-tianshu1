...
优化日志 (2026-02-16):
1. [修复] 强制回写源 PDF 到 output 目录，解决前端无法预览源文件的问题
2. [修复] PaddleOCR/MinerU 返回结果中补全 json_content 和 pdf_path 以支持双向定位
3. [性能] 移除单次任务后的强制显存清理 (clean_memory)，依赖引擎的智能休眠机制
4. [稳定] 增强 VLLM 容器互斥切换的健壮性
"""
    ...
    def _process_task(self, task: dict):
        ...
        try:
            ...
            if backend == "paddleocr-vl-vllm" and self.paddleocr_vl_vllm_api:
                logger.info(f"🔄 Ensuring PaddleOCR-VL VLLM service is running")
                ...
                logger.info(f"🔄 Ensuring MinerU VLLM service is running")
            ...
            # 注意：result 字典中必须包含 pdf_path，否则前端无法预览源文件
            self.task_db.update_task_status(
                ...
                data=json.dumps({
                    ...
                    "markdown_file": result.get("markdown_file") # 可选：下载文件名
                })
            )
